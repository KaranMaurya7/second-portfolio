...
export const Contact = ({ firestore, showNotification }) => {
	...
	const validateForm = () => {
		...
		if (!name || !message) {
			showNotification("Name and Message fields are required.", "error");
			return false;
		}

		if (!phone && !email) {
			showNotification("Either Phone or Email must be provided.", "error");
			return false;
		}
		return true;
	};
	...
	const handleSubmit = async (e) => {
		...
		try {
			await addDoc(collection(firestore, "users", "user1", "contactMessages"), formData);		
